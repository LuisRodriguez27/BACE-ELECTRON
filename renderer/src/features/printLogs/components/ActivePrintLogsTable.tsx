            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left text-gray-600 border-collapse">
                ...
                <tbody className="divide-y divide-gray-100 bg-white">
                  {group.logs.map(log => {
                    const isLate = isBeforeNow(log.hora_entrega) && (!log.maquila_completada || !log.mostrador_completado);
                      ...
                      >
                        ...
                        <td className={`py-3 px-4 whitespace-nowrap ${bgMaq} ${bgMaq ? '' : 'font-medium text-gray-900'}`}>
                          <span className="flex items-center gap-1.5">
                            <Clock size={14} className={isLate ? "text-red-500" : (log.maquila_completada ? "text-emerald-600" : "text-gray-400")} />
                        ...
                        <td className={`py-3 px-4 ${bgMost} ${bgMost ? '' : 'text-gray-600'}`}>
                          <span className="flex items-center gap-1">
                            <Send size={12} className={log.mostrador_completado ? "text-emerald-600" : "text-gray-400"} />
                            {log.envio}
                          </span>
                        </td>
