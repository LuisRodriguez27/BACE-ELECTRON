import { useState } from 'react';
import { toast } from 'sonner';
import { Button } from '@/components/ui/button';
import type { Note } from '../types';
import { captureNoteHtmlAsPng, prepareNoteHtml } from '../utils/buildNotePageHtml';

export function useWhatsAppNote() {
  const [isSendingWhatsApp, setIsSendingWhatsApp] = useState(false);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [messageText, setMessageText] = useState('');
  const [pendingNote, setPendingNote] = useState<Note | null>(null);

  const startWhatsAppFlow = (note: Note) => {
    setMessageText('Se le envía la nota con la información solicitada.');
    setPendingNote(note);
    setIsDialogOpen(true);
  };

  const cancel = () => {
    setIsDialogOpen(false);
    setPendingNote(null);
  };

  const confirmAndSend = async () => {
    if (!pendingNote) return;

    setIsDialogOpen(false);
    setIsSendingWhatsApp(true);

    try {
      const preparedDocument = await prepareNoteHtml(pendingNote, 'whatsapp');
      const blob = await captureNoteHtmlAsPng(preparedDocument);

      try {
        await navigator.clipboard.write([new ClipboardItem({ 'image/png': blob })]);
        const pagesLabel = preparedDocument.pageCount === 1
          ? 'La imagen fue copiada'
          : `Las ${preparedDocument.pageCount} páginas fueron copiadas en una sola imagen`;
        toast.success(`${pagesLabel}. ¡Pégala en WhatsApp con Ctrl+V!`);
      } catch {
        const url = URL.createObjectURL(blob);
        const anchor = document.createElement('a');
        anchor.href = url;
        anchor.download = `nota-${pendingNote.id}.png`;
        anchor.click();
        URL.revokeObjectURL(url);
        toast.info('Imagen descargada. Adjúntala manualmente en WhatsApp.');
      }

      const rawPhone = (pendingNote.phone || '').replace(/\D/g, '');
      const phoneWithCountry = rawPhone.length === 10 ? `52${rawPhone}` : rawPhone;
      const whatsappUrl = phoneWithCountry
        ? `https://web.whatsapp.com/send?phone=${phoneWithCountry}&text=${encodeURIComponent(messageText)}`
        : 'https://web.whatsapp.com/';

      if (!phoneWithCountry) {
        toast.warning('La nota no tiene número registrado. Selecciona el chat manualmente.');
      }
      await window.api.openExternal(whatsappUrl);
    } catch (error) {
      console.error('Error al enviar la nota por WhatsApp:', error);
      toast.error('Ocurrió un error al preparar el envío por WhatsApp.');
    } finally {
      setIsSendingWhatsApp(false);
      setPendingNote(null);
    }
  };

  const whatsappDialogElement = isDialogOpen ? (
    <div
      className="fixed inset-0 flex items-center justify-center z-9999"
      style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}
    >
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg mx-4">
        <h2 className="text-xl font-bold mb-4 text-gray-900">Mensaje de WhatsApp</h2>
        <p className="text-sm text-gray-500 mb-4">
          Edita el mensaje que se enviará al cliente (nota #{pendingNote?.id}).
        </p>
        <textarea
          className="w-full h-32 p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#25d366] focus:border-transparent resize-none"
          value={messageText}
          onChange={event => setMessageText(event.target.value)}
        />
        <div className="flex justify-end gap-3 mt-6">
          <Button variant="outline" onClick={cancel}>Cancelar</Button>
          <Button
            onClick={confirmAndSend}
            className="bg-[#25D366] hover:bg-[#1ebe5d] text-white"
            disabled={isSendingWhatsApp}
          >
            Generar y Enviar
          </Button>
        </div>
      </div>
    </div>
  ) : null;

  return { isSendingWhatsApp, sendWhatsApp: startWhatsAppFlow, whatsappDialogElement };
}
