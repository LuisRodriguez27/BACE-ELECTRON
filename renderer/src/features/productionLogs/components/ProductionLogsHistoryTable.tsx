...
import type { PaginatedHistoryDays, ProductionLog, ProductionLogsTableRef } from '../types';
...
const ProductionLogsHistoryTable = forwardRef<ProductionLogsTableRef, ProductionLogsHistoryTableProps>(
  ({ onEditClick }, ref) => {
    ...
    const [selectedDayLogs, setSelectedDayLogs] = useState<ProductionLog[]>([]);
    ...
    const handlePrintFromCard = async (e: React.MouseEvent, dateStr: string) => {
      ...
      try {
        const logs = await ProductionLogsApiService.getByDay(dateStr);
        handlePrintGroup(logs, dateStr);
      } catch (err) {
        console.error('Error fetching production logs for day print:', err);
    ...
    const fetchHistoryDays = async (page: number) => {
      try {
        setIsLoading(true);
        setError(null);
        const data = await ProductionLogsApiService.getHistoryDays(
          todayLocalStr,
          page,
          historyLimit,
          searchTerm,
    ...
    const fetchDayLogs = async (dateStr: string) => {
      try {
        ...
        console.error('Error fetching production logs for day:', err);
    ...
    useImperativeHandle(ref, () => ({
      ...
      handleLogUpdated: (updatedLog: ProductionLog) => {
            ...
            >
              <div className="flex-1 min-w-[200px]">
                <Label htmlFor="search_input" className="text-xs font-bold text-gray-700 uppercase">
                  Buscar por ID / ID Orden / Cliente / Creador
                </Label>
              ...
              <div className="flex gap-2">
                <Button
                  type="submit"
                  className="font-medium flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white"
                >
                  <Search size={16} />
                  Buscar
                </Button>
              ...
              {historyData.data.map((day) => (
                ...
                >
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between text-sm text-gray-500">
                    <span>{day.total_logs} {day.total_logs === 1 ? 'registro' : 'registros'}</span>
                    <div className="flex items-center gap-2">
                      <Button
                        variant="ghost"
                        size="sm"
                        className="h-8 w-8 p-0 text-gray-500 hover:text-gray-700 hover:bg-gray-100 animate-none"
                        onClick={(e) => handlePrintFromCard(e, day.log_date)}
                        title="Imprimir esta bitácora"
                      >
                        <Printer size={15} />
                      </Button>
                      <span className="text-indigo-600 font-semibold hover:underline">Ver detalles &rarr;</span>
                    </div>
                  </div>
                </div>
              ))}
                      ...
                      return (
                        <React.Fragment key={p}>
                          ...
                          <Button
                            ...
                            className={isCurrent ? "bg-indigo-600 text-white hover:bg-indigo-700" : ""}
                          >
                            {p}
                          </Button>
                        </React.Fragment>
          ...
          >
            <div className="bg-white rounded-xl shadow-2xl max-w-6xl w-full mx-4 overflow-hidden flex flex-col max-h-[85vh]">
              ...
              <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gray-50">
                <div>
                  <h2 className="text-lg font-bold text-gray-900">
                    Registros de Producción del {formatDateHeaderMX(selectedDay)}
                  </h2>
              ...
              <div className="p-6 border-t border-gray-200 bg-gray-50 flex justify-end">
                <Button onClick={() => setSelectedDay(null)} className="bg-indigo-600 hover:bg-indigo-700 text-white font-medium">
                  Cerrar
                </Button>
              </div>
            </div>
...
ProductionLogsHistoryTable.displayName = 'ProductionLogsHistoryTable';

export default ProductionLogsHistoryTable;
