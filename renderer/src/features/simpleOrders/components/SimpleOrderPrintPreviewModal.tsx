...
import {
  buildSimpleOrderPrintHtml,
  prepareSimpleOrderHtml,
  type PreparedSimpleOrderHtml,
...
const SimpleOrderPrintPreviewModal: React.FC<SimpleOrderPrintPreviewModalProps> = ({
  ...
  const [preparedDocument, setPreparedDocument] = useState<PreparedSimpleOrderHtml | null>(null);
  ...
  useEffect(() => {
    ...
    setIsPreparing(true);
    prepareSimpleOrderHtml(orderData)
      .then(documentHtml => {
        if (!cancelled) setPreparedDocument(documentHtml);
      })
      .catch(error => {
        console.error('Error al preparar la vista previa de la orden rápida:', error);
        if (!cancelled) toast.error('No se pudo generar la vista previa de la orden rápida');
      })
      .finally(() => {
  ...
  const handlePrint = () => {
    ...
    try {
      ...
      printWindow.document.write(buildSimpleOrderPrintHtml({
      ...
      console.error('Error al imprimir la orden rápida:', error);
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-7xl w-full mx-4 max-h-[95vh] overflow-hidden">
        ...
        <div className="p-6 overflow-auto max-h-[calc(95vh-100px)]">
          {isPreparing && !preparedDocument ? (
            ...
            <div
              className="flex flex-col items-center gap-8 [&>.simple-order-document-page]:border [&>.simple-order-document-page]:border-gray-300 [&>.simple-order-document-page]:shadow-lg"
