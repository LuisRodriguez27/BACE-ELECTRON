...
class SupplierOrderRepository {
  ...
  async addItemsToOrder(supplierOrderId: number, items: unknown[]): Promise<void> {
  ...
  async findPreviousItemsBySupplier(supplierId: number) {
    const rows = await db.getAll<{ item_data: string | Record<string, unknown> }>(`SELECT soi.item_data FROM supplier_order_items soi JOIN supplier_orders so ON soi.supplier_order_id = so.id WHERE so.supplier_id = $1 AND soi.active = true AND so.active = true ORDER BY soi.id DESC`, [supplierId]);
