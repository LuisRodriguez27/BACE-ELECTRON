...
class PrintLogRepository {
  ...
  async findAllPaginated(page = 1, limit = 10, searchTerm = '', searchDate = null) {
    ...
    if (searchTerm && searchTerm.trim()) {
      ...
      searchCondition += `
        AND (
          CAST(pl.id AS TEXT) = $${paramIndex}
          OR CAST(pl.order_id AS TEXT) = $${paramIndex}
        )
      `;
    ...
    const countQuery = `
      ...
      LEFT JOIN orders o ON pl.order_id = o.id
      LEFT JOIN clients c ON o.client_id = c.id
      WHERE pl.active = TRUE ${searchCondition}
    `;
    ...
    const dataQuery = `
      ...
      LEFT JOIN clients c ON o.client_id = c.id
      WHERE pl.active = TRUE ${searchCondition}
      ORDER BY pl.created_at DESC, pl.hora_entrega DESC
      LIMIT $${paramIndex} OFFSET $${paramIndex + 1}
    `;
    const rows = await db.getAll(dataQuery, [...searchParams, limit, offset]);

    return {
      data: rows.map(r => new PrintLog(r)),
      pagination: {
        page,
        limit,
        total,
  ...
  async getByOrderId(orderId) {
    const rows = await db.getAll(`
      ...
      LEFT JOIN orders o ON pl.order_id = o.id
      LEFT JOIN clients c ON o.client_id = c.id
      WHERE pl.order_id = $1 AND pl.active = TRUE
      ORDER BY pl.hora_entrega DESC
    `, [orderId]);
    return rows.map(r => new PrintLog(r));
  }
  ...
  async create({ order_id, descripcion, hora_entrega, responsable, observaciones, envio, pago, maquila_completada = false, mostrador_completado = false, status = 'Pendiente', created_at }) {
    ...
    const result = await db.execute(`
      INSERT INTO print_logs (${fields.join(', ')})
  ...
  async getHistoryDaysPaginated(todayLocalStr, page = 1, limit = 10, searchTerm = '', searchDate = null) {
    const offset = (page - 1) * limit;
    let searchCondition = '';
    const searchParams = [todayLocalStr];
    let paramIndex = 2;
    ...
    if (searchTerm && searchTerm.trim()) {
      ...
      searchCondition += `
        AND (
          CAST(pl.id AS TEXT) = $${paramIndex}
          OR CAST(pl.order_id AS TEXT) = $${paramIndex}
          OR pl.descripcion ILIKE '%' || $${paramIndex} || '%'
    ...
    const countQuery = `
      SELECT COUNT(DISTINCT TO_CHAR(pl.created_at AT TIME ZONE 'America/Mexico_City', 'YYYY-MM-DD')) as total
      FROM print_logs pl
      LEFT JOIN orders o ON pl.order_id = o.id
      LEFT JOIN clients c ON o.client_id = c.id
      WHERE pl.active = TRUE
        AND TO_CHAR(pl.created_at AT TIME ZONE 'America/Mexico_City', 'YYYY-MM-DD') < $1
        ${searchCondition}
    `;
    const countResult = await db.getOne(countQuery, searchParams);
    const total = parseInt(countResult.total, 10);
    ...
    return {
      data: rows.map(r => ({
        log_date: r.log_date,
        total_logs: parseInt(r.total_logs, 10),
  ...
  async delete(id) {
    const result = await db.execute(`
      UPDATE print_logs SET active = FALSE WHERE id = $1 AND active = TRUE
    `, [id]);
